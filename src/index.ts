...
async function submitUrlForArchiving(url: string, debug = false): Promise<string> {
...
program
    ...
    .action(async (urlArg: string | undefined, opts: { keepProtocol?: boolean, debug?: boolean }) => {
        ...
        if (typeof url === "string" && /^https?:\/\//i.test(url)) {
            log.warn("Warning: Including the protocol (http:// or https://) will usually cause archiving to fail. It's recommended to strip it.");
